...
class Faiss(AutotoolsPackage, CMakePackage, CudaPackage):
    ...
    build_system(
        conditional("cmake", when="@1.7:"),
        conditional("autotools", when="@:1.6"),
        default="cmake",
    )
    ...
    def setup_run_environment(self, env: EnvironmentModifications) -> None:
        if self.spec.satisfies("+python"):
            ...
            if self.spec.satisfies("platform=darwin"):
                ...
                env.append_path(
                    "LD_LIBRARY_PATH", os.path.join(python_platlib, "faiss")
                )
        ...
        if spec.satisfies("+python"):
            ...
            class CustomPythonPipBuilder(python.PythonPipBuilder):
                ...
                def build_directory(self):
                    return os.path.join(
                        self.pkg.stage.path, self.build_dirname, "faiss", "python"
                    )
